...
// kill_on_touch ( other_part )
// kills character touching this object
Blockly.Blocks['kill_on_touch'] = {
	init: function() {
		this.appendValueInput("touchedpart")
			.setCheck(null)
			.appendField("Kill On Touch");
			.appendField(new Blockly.FieldVariable("Touched Part"), "_touched_part");
		this.setPreviousStatement(true, null);
		this.setNextStatement(true, null);
		this.setColour(330);
		this.setTooltip("");
...
lua.luaGenerator.forBlock['kill_on_touch'] = function(block, generator) {
	var variable__touched_part = generator.nameDB_.getName(block.getFieldValue('_touched_part'), Blockly.Variables.NAME_TYPE);
	var value_touchedpart = generator.valueToCode(block, 'touchedpart', lua.Order.ATOMIC);
	var code = 'part = script.Parent part.Touched:Connect(function(${variable__touched_part) ' + 
		'local char_other,humanoid_other char_other = other_part.Parent humanoid_other = char_other:FindFirstChild('Humanoid')' + 
		'if humanoid_other then humanoid_other.Health = 0 end end)';
